...
function printMqttBanner(region, autoReconnect) {
    ...
    var rows = [
        titleClr + '  ✅  toru-fca MQTT Connected' + rst,
        '',
...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
    // Reconnect the MQTT transport only. Do NOT re-login or refresh the
    // Facebook appstate on transient socket failures. This avoids repeated
    // account sessions while keeping the inbox listener alive.
    ctx._reconnectState = ctx._reconnectState || { attempt: 0, timer: null, stopped: false, connectedAt: 0 };
    ctx._reconnectState.stopped = false;
    ...
    function scheduleReconnect(reason) {
        if (!ctx.globalOptions.autoReconnect || ctx._reconnectState.stopped || ctx.loggedIn === false) {
            return globalCallback({ type: "stop_listen", error: reason || "Connection closed" }, null);
        }
        if (ctx._reconnectState.timer) return;
        var min = Number(ctx.globalOptions.reconnectMinDelayMs) || 3000;
        var max = Number(ctx.globalOptions.reconnectMaxDelayMs) || 60000;
        var stable = Number(ctx.globalOptions.reconnectStableResetMs) || 30000;
        var elapsed = ctx._reconnectState.connectedAt ? Date.now() - ctx._reconnectState.connectedAt : 0;
        if (elapsed >= stable) ctx._reconnectState.attempt = 0;
        var attempt = ctx._reconnectState.attempt++;
        var delay = Math.min(max, min * Math.pow(2, Math.min(attempt, 5)));
        delay += Math.floor(Math.random() * Math.min(1000, Math.max(250, Math.floor(delay * 0.15))));
        log.warn("listenMqtt", "MQTT reconnect scheduled in " + delay + "ms" + (reason ? " (" + reason + ")" : ""));
        ctx._reconnectState.timer = setTimeout(function () {
            ctx._reconnectState.timer = null;
            if (ctx._reconnectState.stopped || ctx.loggedIn === false) return;
            getSeqID();
        }, delay);
    }
    ...
    mqttClient.on('error', function (err) {
        stopMqttSpinner();
        log.error("listenMqtt", err && err.message ? err.message : err);
        try { mqttClient.end(); } catch (_) {}
        scheduleReconnect("MQTT error");
    });

    mqttClient.on('close', function () {
        scheduleReconnect("MQTT closed");
    });
    mqttClient.on('offline', function () { });
    mqttClient.on('reconnect', function () { });

    mqttClient.on('connect', function () {
        ctx._reconnectState.connectedAt = Date.now();
        ctx._reconnectState.attempt = 0;
        ...
        var rTimeout = setTimeout(function () {
            try { mqttClient.end(); } catch (_) {}
            scheduleReconnect("sync queue timeout");
        }, 15000);

        ctx.tmsWait = function () {
            clearTimeout(rTimeout);
            ctx.firstListen = false; // ensure reconnects always re-fetch seqID
            if (ctx.globalOptions.emitReady) globalCallback({ type: "ready", error: null });
            delete ctx.tmsWait;

            // ── E2EE bridge init ──────────────────────────────────────────────────
            // Always update ctx._globalCallback so E2EE reconnects use the fresh callback.
            ...
            if (ctx.globalOptions.enableE2EE === true) {
                ...
                // Only initiate connect when the bridge is not already connected/connecting.
                // createBridge() returns the existing bridge; bridge.connect() is idempotent
                // (updates lastGlobalCallback and returns early when already connected),
                // but calling it on every MQTT reconnect produces unnecessary log noise.
                var bridgeState = bridge.getState ? bridge.getState() : {};
                if (!bridgeState.connected && !bridgeState.connectingPromise) {
                    bridge.connect(globalCallback).catch(function (err) {
                        log.error("listenMqtt", "E2EE bridge connect error:",
                            err && err.message ? err.message : err);
                    });
                } else {
                    // Bridge already connected — just refresh the callback reference.
                    if (typeof bridge.connect === "function") {
                        bridge.connect(globalCallback).catch(function () {});
                    }
                }
            } else {
        ...
        if (clientPayload && clientPayload.deltas) {
            for (var i in clientPayload.deltas) {
                ...
                if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
                    (function () {
                        var _rxn = delta.deltaMessageReaction;
                        var _tid = (_rxn.threadKey.threadFbId ? _rxn.threadKey.threadFbId : _rxn.threadKey.otherUserFbId).toString();
                        globalCallback(null, {
