...
/**
 * Custom error class used by FCA src modules.
 * Supports both string and object construction:
 *   new utils.CustomError("message")
 *   new utils.CustomError({ error: "message", code: 123 })
 */
class CustomError extends Error {
    constructor(msgOrObj) {
        if (msgOrObj && typeof msgOrObj === 'object' && !Array.isArray(msgOrObj)) {
            super(msgOrObj.error || msgOrObj.message || JSON.stringify(msgOrObj));
            Object.assign(this, msgOrObj);
        } else {
            super(String(msgOrObj));
        }
        this.name = 'FCAError';
